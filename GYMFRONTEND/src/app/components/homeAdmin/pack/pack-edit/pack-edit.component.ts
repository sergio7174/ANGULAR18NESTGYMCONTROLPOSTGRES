...
getPackData(id:any): void {
    alert("Estoy en editcategory - line 128 - getCategoryData - id: "+ id);

    // get the data, to fill the form 
    this.packService.getpackById(id).pipe(
      takeUntilDestroyed(this.destroyRef)).subscribe((data:any) => {

       alert("Estoy en pack-edit.component - line 134 - data.nameplan: " + data.pack.nameplan);
       
      this.OldImage = data.pack.image;
      
      this.editPackForm.patchValue({

    nameplan:  data.pack.nameplan,
    trialdays: data.pack.trialdays,
    features:  data.pack.features,
    timedays:  data.pack.timedays,
    cost:      data.pack.cost,
    code:      data.pack.code,
