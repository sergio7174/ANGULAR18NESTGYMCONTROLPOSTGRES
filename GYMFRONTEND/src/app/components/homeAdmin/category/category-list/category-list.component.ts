...
/***** End of vars to handle pagination  */
  ...
  getDataCategories() {
    ...
    this.categoryService.getCategoryList().pipe(
      ...
      /*** if there is not Categories at all, show it */
        alert("Estoy en listCategorys-component - line 100 - this.DataCategory.message="+ this.DataCategory.message);
        if (this.filteredItems.total == 0){

          this.toast.error(this.filteredItems.message);
...
deleteDetails (Data:any,index:any,event:any) {
  ...
  this.categoryService.deleteCategory(Data.id,Data.image).subscribe((data) => {
