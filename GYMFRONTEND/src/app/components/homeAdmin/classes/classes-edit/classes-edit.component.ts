...
getClassData(id:any): void {
    ...
    this.classesService.getclassesById(id).pipe(
      takeUntilDestroyed(this.destroyRef)).subscribe((data:any) => {
       ...
       alert("Estoy en classes-edit.component - line 140 - data.name: "+data.clase.classname);
       
      this.OldImage = data.clase.image;
      
      this.editClassForm.patchValue({

    classname:    data.clase.classname,
    code:         data.clase.code,
    classday:     data.clase.classday,
    classtime:    data.clase.classtime,
    classlevel:   data.clase.classname,
    session_time: data.clase.session_time,
    price:        data.clase.price,
    trainer:        data.clase.trainer,
    dateBegin:      data.clase.dateBegin,
    key_benefits:   data.clase.key_benefits,
    expert_trainer: data.clase.expert_trainer,
    class_overview: data.clase.class_overview,
    why_matters:    data.clase.why_matters,
      });
      ...
      /*** service to erase old image  ****************************************/
       ...
       this.classesService.deleteImage(this.OldImage).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (response) => {
          ...
          alert ("Estoy en EditClass - line - 194 - message from backend: " + messageFromBackend );
        }
